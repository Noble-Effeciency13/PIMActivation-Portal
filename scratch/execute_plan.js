...
// 1. Add quickInactivePolicies flag
app = app.replace(
  /showInactivePolicies:\s*false,/,
  `showInactivePolicies: false,
  quickInactivePolicies: false,`
);

// 2. Add flag-show-inactive to showSettingsModal toggle list
app = app.replace(
...
// 3. Add flag-quick-inactive to showSettingsModal checkboxes
app = app.replace(
...
// 4. Update _renderQuickActions
// First remove the old hardcoded quick-labels-btn wrap
app = app.replace(
  /<div class="quick-action-wrap">\s*<button class="icon-btn \${_flags\.showInactivePolicies \? 'active' : ''}" id="quick-labels-btn"[\s\S]*?<\/button>\s*<\/div>/,
...
// Also we need to attach the listener properly inside _renderQuickActions. 
// Previously we appended it via script at the end. We will just use replace to fix it.
app = app.replace(
  /const labelsBtn = document\.getElementById\('quick-labels-btn'\);\s*labelsBtn\?\.addEventListener\('click', \(\) => \{[\s\S]*?\}\);/g,
  ''
);

// Inject listener properly before the close of _renderQuickActions
app = app.replace(
  ...
  labelsBtn?.addEventListener('click', () => {
    ...
    if (sw) { sw.classList.toggle('active', _flags.showInactivePolicies); sw.setAttribute('aria-checked', _flags.showInactivePolicies); }
  });
}`
);

// 5. Add event listeners at the bottom
app = app.replace(
